...
__global__ void Expand_Incoming_Kernel(
    ...
    typename KernelPolicy::Value    *d_beliefs_curr,
    typename KernelPolicy::Value    *d_beliefs_next,
    ...
    static void FullQueue_Core(
        ...
        if (gunrock::oprtr::advance::hasPreScan<AdvanceKernelPolicy::ADVANCE_MODE>()) {
            ...
            enactor_stats->AccumulateEdges(
                    work_progress->template
            GetQueueLengthPointer<unsigned int>(
        ...
        if (!gunrock::oprtr::advance::isFused<AdvanceKernelPolicy::ADVANCE_MODE>()) {
            ...
            if (enactor->debug && (enactor_stats->retval =
                                           util::GRError("filter_forward::Kernel failed", __FILE__, __LINE__)))
                return;
            if (enactor->debug)
            ...
            EnactorStats < SizeT > *enactor_stats)
    {
        ...
        <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream >> >
                                                        (h_data_slice->gpu_idx,
                                                                ...
                                                                h_data_slice->belief_curr.GetPointer(util::DEVICE),
                                                                h_data_slice->belief_next.GetPointer(util::DEVICE),
                                                                h_data_slice->joint_probabilities.GetPointer(
                                                                        util::DEVICE),
...
class BPEnactor :
    public EnactorBase<typename _Problem::SizeT/*, _DEBUG, _SIZE_CHECK*/>
    {
        ...
        /**
         * @brief Enacts BP computing on the specified graph.
         *
         * @tparam AdvanceKernelPolicy Kernel policy for advance operator.
         * @tparam FilterKernelPolicy Kernel policy for filter operator.
         *
        ...
        struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB>
        {
            ...
            {
                return enactor.EnactBP<LB_AdvanceKernelPolicy, FilterKernelPolicy>(src);
            }
            static cudaError_t Init(Enactor &enactor, ContextPtr *context, Problem *problem, int max_grid_size = 0)
            {
                return enactor.InitBP <LB_AdvanceKernelPolicy, FilterKernelPolicy>(
        ...
        struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::TWC_FORWARD>
        {
            static cudaError_t Enact(Enactor &enactor, VertexId src)
            {
                return enactor.EnactBP<TWC_AdvanceKernelPolicy, FilterKernelPolicy>(src);
            }
            static cudaError_t Init(Enactor &enactor, ContextPtr *context, Problem *problem, int max_grid_size = 0)
            {
        ...
        struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT>
        {
            static cudaError_t Enact(Enactor &enactor, VertexId src)
            {
                return enactor.EnactBP<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy>(src);
            }
            static cudaError_t Init(Enactor &enactor, ContextPtr *context, Problem *problem, int max_grid_size = 0)
            {
        ...
        struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_CULL>
        {
            static cudaError_t Enact(Enactor &enactor, VertexId src)
            {
                return enactor.EnactBP<LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(src);
            }
            static cudaError_t Init(Enactor &enactor, ContextPtr *context, Problem *problem, int max_grid_size = 0)
            {
                return enactor.InitBP <LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
                        context, problem, max_grid_size);
            }
        };
        ...
        struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT_CULL>
        {
            static cudaError_t Enact(Enactor &enactor, VertexId src)
            {
                return enactor.EnactBP<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(src);
            }
            static cudaError_t Init(Enactor &enactor, ContextPtr *context, Problem *problem, int max_grid_size = 0)
            {
                return enactor.InitBP <LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
                        context, problem, max_grid_size);
            }
        };
